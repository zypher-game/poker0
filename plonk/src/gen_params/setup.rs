...
use poker_plonk::gen_params::params::get_verifier_params;
use serde::Serialize;
use std::{
...
fn main() {
    ...
    match action {
        POKERVK { directory } => gen_vk_specific(directory),
    };
}

// cargo run --release --features="gen no_vk" --bin gen-params pokervk "./parameters"
fn gen_vk_specific(directory: PathBuf) {
    let params = get_verifier_params().unwrap();
    println!(
        "the size of the constraint system: {}",
